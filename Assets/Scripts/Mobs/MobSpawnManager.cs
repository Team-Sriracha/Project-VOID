...
public class MobSpawnManager : NetworkBehaviour
{
    ...
    private Canvas FindUICanvas()
    {
        ...
        // 1. UIManager의 Canvas 우선 사용 (스케일 일치)
        if (UIManager.Instance != null && UIManager.Instance.MainCanvas != null)
        {
            _uiCanvas = UIManager.Instance.MainCanvas;
            return _uiCanvas;
        }
    ...
    private void CreateOverheadUI(NetworkObject mob)
    {
        ...
        Transform parent = _uiCanvas.transform;
        if (UIManager.Instance != null && UIManager.Instance.OverheadUIContainer != null)
        {
            parent = UIManager.Instance.OverheadUIContainer;
        }
