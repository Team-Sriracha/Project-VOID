...
public class MobAI : NetworkBehaviour
{
    ...
    private float _attackDamageTime;
    ...
    private void ProcessAttackState()
    {
        ...
        if (_attackDamageTime > 0 && Time.time >= _attackDamageTime)
        {
            ApplyAttackDamage();
        }
    }
    ...
    private void DetectNearbyPlayers()
    {
        ...
        LogDebug($"DetectNearbyPlayers: OverlapSphere 결과 = {count}개, Range = {_data.DetectionRange}");
    ...
    private void TryAttack()
    {
        ...
        _attackDamageTime = Time.time + _data.AttackAnimationDuration;
