...
public class MobCombat : NetworkBehaviour, IDamageable
{
    ...
    [Header("이펙트")]
    [Tooltip("공격 이펙트가 발생할 위치. 비어 있으면 몹 루트를 사용합니다.")]
    ...
    public Transform AttackEffectPoint => _attackEffectPoint != null ? _attackEffectPoint : transform;
    ...
    private void OnHpChanged(float prev, float next, bool asServer)
    {
        ...
        if (next <= 0f && prev > 0f)
        {
            AudioManager.Instance?.PlayAttachedOneShot(_data.DeathAudioCue, transform, Vector3.up);
            return;
        }

        if (next < prev)
        {
            AudioManager.Instance?.PlayAttachedOneShot(_data.HitAudioCue, transform, Vector3.up);
        }
    }
    ...
    private float CalculateFinalDamage(float damage)
    {
        ...
        float finalDefense = _data.Defense;
