...
public class PlayerController : TickNetworkBehaviour
{
    ...
    public float GetCurrentMoveSpeed()
    {
        ...
        return _playerStats.MoveSpeed * _moveSpeedMultiplier;
    }
    ...
    private void CollectInput()
    {
        ...
        if (isNotPlaying || (combat != null && !combat.IsAlive))
        {
        ...
        if (inputHandler != null)
        {
            ...
            _inputFireHeld = inputHandler.IsFirePressed();
    ...
    private void ProcessRotation(MoveReplicateData rd, float delta)
    {
        ...
        // Case 2: 비조준 + 발사 - 즉시 발사 방향으로 회전
        else if (rd.FireHeld && rd.AimDirection.sqrMagnitude > 0.01f)
        {
            transform.rotation = Quaternion.LookRotation(rd.AimDirection.normalized);
        }
    ...
    private void StartDashMovement(Vector3 moveDirection, uint currentTick)
    {
        ...
        // 쿨다운 설정
        uint dashCooldownTicks = (uint)Mathf.CeilToInt(_playerStats.DashCooldown * TimeManager.TickRate);
