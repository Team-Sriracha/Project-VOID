...
public class PlayerCardSystem : NetworkBehaviour
{
    ...
    /// <summary>
    /// 속도 고정값 보너스 (CSP 동기화용 - 즉시 반영)
    /// </summary>
    public readonly SyncVar<float> SyncSpeedBonus = new(0f);
    ...
    private ICardModifier ApplyCardEffectInternal(CardData card)
    {
        ...
        bool isStatBoost = modifier is StatBoostModifierBase || modifier is FixedStatBoostModifierBase;
        ...
        if (modifier != null)
        {
            ...
            if (modifier is StatBoostModifierBase statModifier)
            {
                UpdateStatBonus(statModifier.TargetStat, statModifier.GetStatBonus(statModifier.TargetStat));
            }
            // 고정값 스탯 부스트 Modifier인 경우
            else if (modifier is FixedStatBoostModifierBase fixedModifier)
            {
                UpdateStatBonus(fixedModifier.TargetStat, fixedModifier.GetStatBonus(fixedModifier.TargetStat));
            }
    ...
    private void UpdateStatBonus(StatType stat, float bonusToAdd)
    {
        ...
        // Speed 타입인 경우 SyncVar 즉시 업데이트 (고정값 동기화)
        if (stat == StatType.Speed && IsServerInitialized)
        {
            SyncSpeedBonus.Value = newBonus;
        }

        // Health 타입은 OnAcquired(서버) 또는 RebuildStats(클라이언트)에서 처리
        // 여기서 OnMaxHealthModified를 호출하면 이중 회복 발생
    }
    ...
    private List<CardData> GetAvailableCards()
    {
        ...
        foreach (var card in _cardConfig.AllCards)
        {
            ...
            bool isStatBoost = (tempModifier is StatBoostModifierBase) || (tempModifier is FixedStatBoostModifierBase);

            if (tempModifier is StatBoostModifierBase statModifier)
            {
            ...
            else if (tempModifier is FixedStatBoostModifierBase fixedModifier)
            {
                float currentBonus = _statBonuses.GetValueOrDefault(fixedModifier.TargetStat, 0f);
                float maxBonus = _cardConfig.GetMaxBonus(fixedModifier.TargetStat);
    ...
    private void ApplyCard(CardData card)
    {
        ...
        // HealthBoost는 OnAcquired에서 자동으로 HP 회복 처리
        if (modifier != null)
        {
    ...
    /// <summary>
    /// 속도 고정값 보너스 (PlayerController에서 사용)
    /// </summary>
    public float GetSpeedBonus()
    {
        // SyncVar 값 직접 사용 (CSP 동기화를 위해 즉시 반영)
        return SyncSpeedBonus.Value;
    }

    /// <summary>
    /// HP 고정값 보너스 (PlayerCombat에서 사용)
    /// </summary>
    public float GetHealthBonus()
    {
        return GetStatBonus(StatType.Health);
    }
