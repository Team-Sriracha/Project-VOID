...
public class PlayerCombat : NetworkBehaviour, IDamageable
{
    ...
    [SerializeField] private AudioCue _healAudioCue;
    [SerializeField] private AudioCue _deathAudioCue;
    [SerializeField] private AudioCue _victoryAudioCue;
    [SerializeField] private AudioCue _levelUpAudioCue;
    [SerializeField] private AudioCue _killConfirmAudioCue;

    [Header("레벨업 이펙트")]
    [SerializeField] private GameObject _levelUpEffectPrefab;
    [SerializeField] private float _levelUpEffectDuration = 2f;
    [SerializeField] private Vector3 _levelUpEffectOffset = Vector3.zero;
    ...
    public float MaxHP
    {
        get
        {
            ...
            // PlayerCardSystem이 있으면 고정값 보너스 적용
            var cardSystem = GetComponent<PlayerCardSystem>();
            if (cardSystem != null)
            {
                baseMaxHP += cardSystem.GetHealthBonus();
            }
    ...
    private void OnHpChanged(float prev, float next, bool asServer)
    {
        ...
        if (next > prev)
        {
            AudioManager.Instance?.PlayAttachedOneShot(_healAudioCue, transform, Vector3.up);
            return;
        }

        if (next <= 0f && prev > 0f)
        {
            AudioManager.Instance?.PlayAttachedOneShot(_deathAudioCue, transform, Vector3.up);
            return;
        }

        if (next < prev)
        {
            AudioManager.Instance?.PlayAttachedOneShot(_hitAudioCue, transform, Vector3.up);
        }
    }
    ...
    private void ApplyDamageInternal(float damage, NetworkConnection attacker, Vector3 indicatorPosition)
    {
        ...
        {
            // 스킬 사용 중이면 히트 애니메이션 건너뜀 (슈퍼아머)
            // IsUsingSkill: 스킬 발동 시점부터 true (OnMeleeSkillMoveStart 전에도 적용)
            var playerController = GetComponent<PlayerController>();
            var weapon = GetComponent<NetworkedWeapon>();
            bool isSkillActive = (playerController != null && playerController.IsSkillMoving) 
                              || (weapon != null && weapon.IsUsingSkill.Value);
            
            // 공격 중(Melee)이거나 스킬 사용 중이면 히트 애니메이션 건너뜀 (슈퍼아머 & 공격 우선)
            bool isAttackActive = (weapon != null && weapon.IsAttacking.Value);

            if (!isSkillActive && !isAttackActive)
            {
    ...
    private void Die(NetworkConnection killer)
    {
        ...
        int defeatRank = CalculateCurrentDefeatRank();
        // 서버에서 확정된 순위를 TargetRpc로 직접 전달하여 클라이언트 동기화 타이밍 오차를 제거
        RPC_ShowDefeatUI(Owner, KillCount.Value, defeatRank);
        ...
        var networkGameStateManager = CustomNetworkManager.Instance?.GameStateManager;
        if (networkGameStateManager != null)
        {
            networkGameStateManager.OnPlayerDied(Owner, killer, killerWeaponID);
        }
    }
    ...
    private int CalculateCurrentDefeatRank()
    {
        ...
        return Mathf.Max(1, aliveCount + 1);
    }
    ...
    private void LevelUp(float xpToNextLevel)
    {
        ...
        RPC_PlayLevelUpAudio(Owner);
        RPC_PlayLevelUpEffect();
    }
    ...
    public void AddPlayerKill()
    {
        ...
        RPC_PlayKillConfirmAudio(Owner);
    }
    ...
    /// </summary>
    [TargetRpc]
    private void RPC_ShowDefeatUI(NetworkConnection target, int killCount, int rank)
    {
        if (UIManager.Instance != null)
        {
            UIManager.Instance.ShowDefeat(killCount, rank);
        }
    }

    [TargetRpc]
    private void RPC_PlayLevelUpAudio(NetworkConnection target)
    {
        AudioManager.Instance?.PlayUi(_levelUpAudioCue);
    }

    [TargetRpc]
    private void RPC_PlayKillConfirmAudio(NetworkConnection target)
    {
        AudioManager.Instance?.PlayUi(_killConfirmAudioCue);
    }

    [ObserversRpc]
    private void RPC_PlayLevelUpEffect()
    {
        ...
        Vector3 effectPosition = transform.position + _levelUpEffectOffset;
        GameObject levelUpEffect = Instantiate(
            _levelUpEffectPrefab,
            effectPosition,
            _levelUpEffectPrefab.transform.rotation,
            transform);

        Destroy(levelUpEffect, _levelUpEffectDuration);
    }

    [ObserversRpc]
    private void RPC_PlayVictoryAudio()
    {
        AudioManager.Instance?.PlayAttachedOneShot(_victoryAudioCue, transform, Vector3.up);
    }
