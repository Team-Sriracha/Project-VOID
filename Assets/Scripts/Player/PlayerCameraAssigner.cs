...
/// Fishnet Server 모드에서 Owner를 가진 클라이언트만 카메라를 생성합니다.
/// </summary>
public class PlayerCameraAssigner : NetworkBehaviour
{
    #region Constants

    private static readonly Vector3 AUDIO_LISTENER_OFFSET = new(0f, 1.6f, 0f);
    ...
    private GameObject _audioListenerAnchor;
    ...
    public override void OnStartClient()
    {
        ...
        // Why: Owner를 가진 클라이언트(로컬 플레이어)만 카메라 생성
        if (!IsOwner)
            return;
    ...
    public override void OnStopClient()
    {
        ...
        try
        {
            // Debug: 어떤 플레이어가 Stop되는지 확인
            int ownerId = OwnerId;
            bool isLocal = IsOwner;
            Debug.LogWarning($"[PlayerCameraAssigner] OnStopClient called - OwnerId: {ownerId}, IsLocalPlayer: {isLocal}");
        ...
        {
            Debug.LogError($"[PlayerCameraAssigner] Error in OnStopClient: {ex}");
        }
    }
    ...
    private void CreateCamera()
    {
        ...
        _cameraTarget = new GameObject($"CameraTarget_{OwnerId}");
        ...
        _localCamera.name = $"PlayerCamera_{OwnerId}";
    ...
    private void DestroyCamera()
    {
        ...
        if (_audioListenerAnchor != null)
        {
            Destroy(_audioListenerAnchor);
            _audioListenerAnchor = null;
            _audioListener = null;
        }
    ...
    {
        UpdateAudioListenerAnchor();
    }

    private void CreateAudioListenerAnchor()
    {
        _audioListenerAnchor = new GameObject($"AudioListener_{OwnerId}");
        _audioListener = _audioListenerAnchor.AddComponent<AudioListener>();
        UpdateAudioListenerAnchor();
    }

    private void UpdateAudioListenerAnchor()
    {
        if (_audioListenerAnchor == null)
        {
            return;
        ...
        _audioListenerAnchor.transform.position = transform.position + AUDIO_LISTENER_OFFSET;

        if (_localCamera != null)
        {
            _audioListenerAnchor.transform.rotation = _localCamera.transform.rotation;
        }
    }
