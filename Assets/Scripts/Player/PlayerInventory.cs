...
public class PlayerInventory : NetworkBehaviour
{
    ...
    public void RPC_RequestSwapItem(int slotIndex, int itemObjectId, NetworkConnection sender = null)
    {
        ...
        Vector3 pickupPosition = item.transform.position;
        string pickupItemId = item.ItemID.Value;

        SwapItem(slotIndex, item);

        if (sender != null)
        {
            // Why: 픽업음은 로컬 피드백이므로 실제 픽업을 확정한 플레이어에게만 전달합니다.
            TargetRpc_PlayPickupAudio(sender, pickupItemId, pickupPosition);
        }
    }
    ...
    /// </summary>
    public NetworkedItem GetNetworkedItemById(int objectId)
    {
        ...
        // [Add] ClientManager에서도 검색 (클라이언트 로직용)
        if (InstanceFinder.ClientManager != null &&
            InstanceFinder.ClientManager.Objects.Spawned.TryGetValue(objectId, out NetworkObject netObjClient))
        {
            return netObjClient.GetComponent<NetworkedItem>();
        }
    ...
    /// <summary>
    /// 아이템 타입에 해당하는 슬롯에 장착된 NetworkedItem을 반환합니다.
    /// </summary>
    public NetworkedItem GetEquippedItem(ItemType itemType)
    {
        int slotIndex = GetSlotForItemType(itemType);
        if (slotIndex == -1) return null;

        int itemId = GetItemSlotObjectId(slotIndex);
        if (itemId == 0) return null;

        return GetNetworkedItemById(itemId);
    }
