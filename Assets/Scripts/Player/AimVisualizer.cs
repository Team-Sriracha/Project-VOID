...
/// 조준선을 시각화합니다.
/// 세그먼트별 장애물 감지를 구현합니다.
/// Multi-Peer 환경에서 올바른 Physics Scene을 사용합니다.
/// </summary>
public class AimVisualizer : MonoBehaviour
{
    ...
    // Why: Multi-Peer 환경에서 올바른 Physics Scene 사용
    ...
    /// <summary>
    /// Multi-Peer 환경에서 올바른 Physics Scene을 가져옵니다.
    /// </summary>
    private void UpdatePhysicsScene()
    {
        ...
        if (_networkObject != null && _networkObject.Runner != null && 
            _networkObject.Runner.SceneManager != null)
        {
            if (_networkObject.Runner.SceneManager.TryGetPhysicsScene3D(out var scene) && scene.IsValid())
            {
                _physicsScene = scene;
    ...
    /// <summary>
    /// 올바른 Physics Scene에서 Raycast를 수행합니다.
    /// </summary>
    private bool DoRaycast(Vector3 origin, Vector3 direction, out RaycastHit hit, float maxDistance, LayerMask layerMask)
    {
        // Multi-Peer: 올바른 Physics Scene 사용
        if (_hasValidPhysicsScene && _physicsScene.IsValid())
        {
            return _physicsScene.Raycast(origin, direction, out hit, maxDistance, layerMask);
        }

        // Fallback: 기본 Physics (Single-Peer 또는 Physics Scene 없는 경우)
    ...
    {
        // Why: Physics Scene이 아직 설정되지 않았으면 다시 시도
        if (!_hasValidPhysicsScene)
        {
            UpdatePhysicsScene();
        }

        // Why: AimPoint의 회전을 반영한 방향 사용
