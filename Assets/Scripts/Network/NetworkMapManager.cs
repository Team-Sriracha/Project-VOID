...
namespace ProjectVoid.Network
{
    public class NetworkMapManager : NetworkBehaviour
    {
        ...
        {
            Debug.Log($"[NetworkMapManager] InitializeMap called - Mode: {mode}, Players: {playerCount}");
            ...
            _selectedTemplate = templates[Random.Range(0, templates.Length)];
            ...
            for (int i = 0; i < allSpawnPoints.Length; i++)
            {
                if (allSpawnPoints[i].IsValid)
                {
                    if (validIndex == targetValidIndex)
                    {
                        ...
                        Vector3 worldPosition = GridToWorldPosition(gridPosition.x, gridPosition.y, doorOffset);

                        Debug.Log($"[NetworkMapManager] Player {playerIndex} 스폰 위치: {worldPosition}");
