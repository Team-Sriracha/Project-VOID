...
/// <summary>
/// 몹 오버헤드 UI 관리 (이름, HP 바)
/// PlayerOverheadUI와 동일하게 Screen Space Overlay 방식으로, 게임 메인 UI 뒤에서 표시합니다.
/// </summary>
public class MobOverheadUI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // [FIX] 오버헤드 UI가 마우스 입력을 가로채지 않도록 설정
    ...
    private void Update()
    {
        ...
        // 성능 최적화: 0.033초(~30fps) 간격으로만 업데이트
    ...
    private void UpdateHealthBar()
    {
        ...
        float hpRatio = _mobData.MaxHP > 0
            ? _combat.HP.Value / _mobData.MaxHP
            : 0f;

        _hpBar.value = hpRatio;
        if (_hpText != null)
        {
            _hpText.text = $"{Mathf.CeilToInt(_combat.HP.Value)}";
        }
    }

    /// <summary>
    /// 몹 월드 좌표를 스크린 좌표로 변환, UI 위치 업데이트
    ...
    private void UpdateScreenPosition()
    {
        ...
        // 화면 경계 밖이면 UI 숨김 (갑자기 나타나는 현상 방지)
