    ...
    private IEnumerator WaitForMapReadyCoroutine()
    {
        ...
        while (mapManager == null && waitTime < maxWaitTime)
        {
            ...
            if (networkManager != null && networkManager.HasSpawnedMapManager && networkManager.NetworkMapManager != null)
            {
                ...
                if (mgr.NetworkObject != null && mgr.NetworkObject.IsSpawned)
                {
                    mapManager = mgr;
        ...
        // Networked 속성은 Spawned() 이후에만 접근 가능하므로 IsSpawned 확인
        bool isValid = mapManager.NetworkObject != null && mapManager.NetworkObject.IsSpawned;
        Debug.Log($"[LoadingUIManager] NetworkMapManager 발견 - IsSpawned: {isValid}, IsMapReady: {(isValid ? mapManager.IsMapReady.Value.ToString() : "N/A")}");
        UpdateLoadingText("맵 생성 중...");

        // Networked 속성은 Spawned() 이후에만 접근 가능
        while (mapManager.NetworkObject == null || !mapManager.NetworkObject.IsSpawned || !mapManager.IsMapReady.Value || !mapManager.IsReady())
        {
            // Object가 아직 유효하지 않으면 대기
            if (mapManager.NetworkObject == null || !mapManager.NetworkObject.IsSpawned)
            {
                yield return new WaitForSeconds(checkInterval);
                continue;
            }
