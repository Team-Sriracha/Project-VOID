...
public class MatchmakingManager : MonoBehaviour, INetworkRunnerCallbacks
{
    ...
    // Why: JoinCustomRoom에서 FindSessionByRoomCode 결과 캐시 (중복 검색 방지)
    private SessionInfo _cachedTargetSession = null;
    ...
    public async void StartMatchmaking(GameMode mode)
    {
        ...
        try
        {
            ...
            await ConnectToGameServer(serverInfo.Value, availableSession);
        }
        catch (Exception ex)
        {
    ...
    public async void JoinCustomRoom(string roomCode)
    {
        ...
        while (elapsedTime < _sessionSearchTimeout && !_isCancelling)
        {
            ...
            try
            {
                // Why: 세션을 직접 찾아서 캐시에 저장 (FindAllGameSessions 중복 호출 방지)
                var foundSession = await FindSessionByRoomCode(roomCode);

                if (foundSession != null)
                {
                    sessionName = foundSession.Name;
                    _cachedTargetSession = foundSession;  // 나중에 사용하기 위해 캐시
        ...
        try
        {
            // Why: 이미 캐시된 세션 정보 사용 (FindAllGameSessions 중복 호출 방지)
            var targetSession = _cachedTargetSession;
            _cachedTargetSession = null;  // 사용 후 캐시 클리어
            ...
            _currentRoomCode = roomCode;
            await ConnectToGameServer(serverInfo.Value, targetSession);
    ...
    /// <summary>
    /// 룸 코드로 세션을 찾아 SessionInfo를 반환합니다.
    /// Why: 세션 이름만 반환하면 다시 FindSpecificGameSession에서 FindAllGameSessions를 호출해야 하므로
    ///      SessionInfo를 직접 반환하여 중복 호출 방지
    /// </summary>
    private async Task<SessionInfo> FindSessionByRoomCode(string roomCode)
    {
        // Why: 캐시에 이미 있으면 세션 목록에서 찾기
        if (_roomCodeToSessionMap.ContainsKey(roomCode))
        {
            string cachedSessionName = _roomCodeToSessionMap[roomCode];
            var cachedSession = _availableSessions.FirstOrDefault(s => s.Name == cachedSessionName);
            if (cachedSession != null && cachedSession.PlayerCount < cachedSession.MaxPlayers)
            {
        ...
        foreach (var session in allSessions)
        {
            ...
            if (session.Properties != null && session.Properties.TryGetValue("RoomCode", out var roomCodeProp))
            {
                ...
                if (string.Equals(sessionRoomCode, roomCode, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    // Why: 인원이 가득 찼으면 null 반환
                    if (session.PlayerCount >= session.MaxPlayers)
                    {
                        Debug.Log($"[Matchmaking] Session {session.Name} is full ({session.PlayerCount}/{session.MaxPlayers})");
                        return null;
                    }
                    return session;
                }
    ...
    private void ResetState()
    {
        ...
        _cachedTargetSession = null;  // Why: 캐시된 세션 정보 클리어
    ...
    private async Task ConnectToGameServer(ServerInfo serverInfo, SessionInfo sessionInfo)
    {
        ...
        // Why: Session Properties에서 직접 연결 정보 및 서버 원본 MaxPlayers 추출
        string directServerIP = "";
        int directServerPort = 27015;
        int serverMaxPlayers = 8;  // 기본값

        if (sessionInfo.Properties != null)
        {
            if (sessionInfo.Properties.TryGetValue("ServerIP", out var ipProp))
                directServerIP = ipProp.PropertyValue?.ToString() ?? "";
            if (sessionInfo.Properties.TryGetValue("ServerPort", out var portProp))
                directServerPort = portProp.IsInt ? (int)portProp.PropertyValue : 27015;
            // Why: Session Properties에서 서버가 설정한 원본 MaxPlayers 값 사용
            // sessionInfo.MaxPlayers는 Photon이 서버를 1명으로 카운트하여 +1된 값이므로 부정확함
            if (sessionInfo.Properties.TryGetValue("MaxPlayers", out var maxPlayersProp))
                serverMaxPlayers = maxPlayersProp.IsInt ? (int)maxPlayersProp.PropertyValue : 8;
        }

        bool useDirectConnection = !string.IsNullOrEmpty(directServerIP);
        Debug.Log($"[Matchmaking] Direct Connection: {useDirectConnection}, IP: {directServerIP}, Port: {directServerPort}, ServerMaxPlayers: {serverMaxPlayers}");

        // Why: 서버 접속 정보 저장
        // MaxPlayers는 서버가 Session Properties에 저장한 원본 값 사용 (Direct Connection 호환)
        // 게임 모드별 목표 인원(_targetPlayerCount)은 접속 후 RPC로 서버에 전달됨
        _pendingGameConnection = new GameConnectionInfo
        {
            SessionName = sessionInfo.Name,
            ServerInfo = serverInfo,
            GameMode = _currentGameMode,
            MaxPlayers = serverMaxPlayers,  // Session Properties의 원본 MaxPlayers 사용 (Direct Connection 호환)
            DirectServerIP = directServerIP,
            DirectServerPort = directServerPort
        };

        Debug.Log($"[Matchmaking] Lobby에서 세션 접속 시작: {sessionInfo.Name} (ServerMaxPlayers: {serverMaxPlayers}, TargetPlayers: {_targetPlayerCount})");
    ...
    private async Task JoinServerFromLobby()
    {
        ...
        // Why: 직접 연결 설정 - ServerIP가 있으면 직접 연결, 없으면 Photon Relay
        bool useDirectConnection = connectionInfo.UseDirectConnection;
        NetAddress serverAddress = default;

        if (useDirectConnection)
        {
            try
            {
                string serverIP = connectionInfo.DirectServerIP;

                // Why: 도메인 이름이면 IP로 resolve
                if (!System.Net.IPAddress.TryParse(serverIP, out _))
                {
                    Debug.Log($"[Matchmaking] Resolving domain: {serverIP}");
                    var hostEntry = System.Net.Dns.GetHostEntry(serverIP);
                    if (hostEntry.AddressList.Length > 0)
                    {
                        ...
                        foreach (var addr in hostEntry.AddressList)
                        {
                            if (addr.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork)
                            {
                                ...
                                Debug.Log($"[Matchmaking] Resolved to IPv4: {serverIP}");
                                break;
                            }
                        }
                    }
                ...
                serverAddress = NetAddress.CreateFromIpPort(serverIP, (ushort)connectionInfo.DirectServerPort);
                Debug.Log($"[Matchmaking] Using Direct Connection: {serverIP}:{connectionInfo.DirectServerPort}");
            }
            catch (System.Exception ex)
            {
                Debug.LogError($"[Matchmaking] Failed to resolve server address: {ex.Message}");
                Debug.Log("[Matchmaking] Falling back to Photon Relay");
                useDirectConnection = false;
            }
        }
        else
        {
            Debug.Log("[Matchmaking] Using Photon Relay (no ServerIP configured)");
        }

        var args = new StartGameArgs
        {
            GameMode = Fusion.GameMode.Client,
            // Why: Photon 공식 문서에 따르면 클라이언트는 SessionName만 지정
            // Photon Cloud가 서버의 reflexive address를 자동으로 전달
            // 직접 연결 시도 후 실패하면 자동으로 Photon Relay로 fallback
            SessionName = connectionInfo.SessionName,
            // Why: 클라이언트는 Address를 직접 지정하지 않음
            // Photon Cloud가 STUN을 통해 획득한 서버 주소를 자동 전달
            // (서버 측에서만 Address = NetAddress.Any(port)로 리스닝 포트 지정)
            // Why: NetworkSceneManagerDefault를 사용하여 서버의 씬 전환을 자동으로 동기화받음
            SceneManager = runnerGo.AddComponent<NetworkSceneManagerDefault>(),
        };

        Debug.Log($"[Matchmaking] Joining session: {connectionInfo.SessionName} (Direct: {useDirectConnection})");
    ...
    private IEnumerator MigrateToSession(SessionInfo targetSession)
    {
        ...
        if (serverInfo != null)
        {
            // Why: Session Properties에서 직접 연결 정보 추출
            string directServerIP = "";
            int directServerPort = 27015;

            if (targetSession.Properties != null)
            {
                if (targetSession.Properties.TryGetValue("ServerIP", out var ipProp))
                    directServerIP = ipProp.PropertyValue?.ToString() ?? "";
                if (targetSession.Properties.TryGetValue("ServerPort", out var portProp))
                    directServerPort = portProp.IsInt ? (int)portProp.PropertyValue : 27015;
            }
