...
public class NetworkedItem : NetworkBehaviour
{
    ...
    private void Update()
    {
        ...
        else
        {
            ...
            // 착지한 상태면 떠다니기 효과 적용
            if (HasLanded.Value && DroppedPosition.Value != Vector3.zero)
            {
    ...
    private void UpdateAttachment()
    {
        ...
        if (_currentAttachPoint != attachPoint)
        {
            ...
            // 부착 시 NetworkedWeapon에 알림 (FirePoint 갱신용)
            if (OwnerConnection.Value != null && OwnerConnection.Value.FirstObject != null)
            {
                var weapon = OwnerConnection.Value.FirstObject.GetComponent<NetworkedWeapon>();
                if (weapon != null)
                {
