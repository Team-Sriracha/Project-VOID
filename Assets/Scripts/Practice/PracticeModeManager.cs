...
namespace ProjectVoid.Practice
{
    public class PracticeModeManager : NetworkBehaviour
    {
        ...
        private void InitializeGameState()
        {
            ...
            string selectedPracticeTemplateName = ResolvePracticeMapTemplateName();

            // Practice Mode는 Server에서 직접 실행되므로 RPC 대신 직접 호출
            // RPC_SetGameModeInfo는 클라이언트가 아직 활성화되지 않아 실패할 수 있음
            // 대신 서버에서 직접 SetPendingMapInit 호출하여 GameModeConfig 로드
            _spawnedGameStateManager.SetPendingMapInit(GameMode.PracticeRange, 1, selectedPracticeTemplateName);
            
            // SyncVar 직접 설정 (서버이므로 가능)
            // TargetPlayerCount는 SyncVar의 직접 접근이 제한될 수 있으므로 SetPendingMapInit에서 처리
        ...
        private void InitializeMap()
        {
            ...
            _spawnedMapManager.InitializeMap(GameMode.PracticeRange, 1, selectedPracticeTemplateName);
            Debug.Log($"[PracticeModeManager] Map initialization started. Template: {selectedPracticeTemplateName}");
        }

        private static string ResolvePracticeMapTemplateName()
        {
            ...
            GameConnectionInfo? currentConnectionInfo = global::NetworkManager.Instance.GetCurrentConnectionInfo();
            if (!currentConnectionInfo.HasValue)
            {
                return string.Empty;
            }

            string mapTemplateName = currentConnectionInfo.Value.CustomSettings.MapTemplateName;
            return string.IsNullOrWhiteSpace(mapTemplateName) ? string.Empty : mapTemplateName.Trim();
        }
        ...
        private void SpawnPlayer()
        {
            ...
            if (targetConn != null)
            {
                ...
                {
                    if (_spawnedGameStateManager.ConnectedPlayers.Value <= 0)
                    {
                        _spawnedGameStateManager.OnPlayerJoined();
                    }

                    if (_spawnedGameStateManager.AlivePlayers.Value <= 0)
                    {
