...
public class FOVCalculator
{
    ...
    public void Initialize(LayerMask obstacleLayers)
    {
    ...
    {
        if (Physics.Raycast(rayStart, direction, out RaycastHit hit, maxRange, _obstacleLayers))
        {
            Vector2 originXZ = new(origin.x, origin.z);
            Vector2 hitXZ = new(hit.point.x, hit.point.z);
            return Vector2.Distance(originXZ, hitXZ);
        }
        return maxRange;
    }
