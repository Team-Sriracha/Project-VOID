...
public class NavMeshBaker : NetworkBehaviour
{
    ...
    private const string MAP_OBJECT_LAYER_NAME = "MapObject";
    ...
    public void BakeNavMeshForMap(MapGenerator mapGenerator)
    {
        ...
        int obstacleColliderCount = 0;
        LayerMask obstacleLayers = GetObstacleLayerMask();
        ...
        if (_includeWalls)
        {
            ...
            foreach (var kvp in mapGenerator.PlacedChunks)
            {
                ...
                foreach (var collider in wallColliders)
                {
                    ...
                    obstacleColliderCount++;
                }
            }

            LogDebug($"장애물 소스 추가: {obstacleColliderCount}개 Collider 수집");
        }
    ...
    {
        LayerMask obstacleLayers = _wallLayer;
        int mapObjectLayer = LayerMask.NameToLayer(MAP_OBJECT_LAYER_NAME);

        if (mapObjectLayer >= 0)
        {
            obstacleLayers |= 1 << mapObjectLayer;
        }
        else
        {
            LogDebug($"'{MAP_OBJECT_LAYER_NAME}' 레이어를 찾지 못해 기존 Wall 마스크만 사용합니다.");
        }
