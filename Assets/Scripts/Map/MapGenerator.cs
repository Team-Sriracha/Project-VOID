...
namespace ProjectVoid.Map
{
    ...
    public class MapGenerator
    {
        ...
        /// <summary>
        /// 현재 생성된 맵의 가로 크기 (청크 개수 단위)
        /// </summary>
        public int MapWidth { get; private set; }
        
        /// <summary>
        /// 현재 생성된 맵의 세로 크기 (청크 개수 단위)
        /// </summary>
        ...
        public NetworkChunkData[] ToNetworkArray()
        {
            ...
            var networkChunks = new System.Collections.Generic.List<NetworkChunkData>();
        ...
        /// <summary>
        /// 여러 청크를 한 번에 비활성화하고 특정 Start Chunk와의 연결성을 검증합니다.
        /// </summary>
        ...
        public HashSet<Vector2Int> GetEdgeChunks()
        {
            ...
            // Why: 맵 외부(PlacedChunks에 없는 공간)와 접하는 청크가 진짜 가장자리
            foreach (var chunkPos in _activeChunks)
            {
                ...
                foreach (var direction in DirectionExtensions.GetAllDirections())
                {
                    ...
                    // Why: PlacedChunks에 없음 = 처음부터 생성되지 않은 맵 외부
            ...
            foreach (var segment in wallSegments)
            {
                ...
                if (segment.IsDoor)
                {
                    chunk.SetDoor(direction, chunk.ChunkData.DoorPrefab, _settings.ChunkSize,
                                chunk.ChunkData.WallSpacing, segment.StartIndex, segment.EndIndex + 1);
                }
                else
                {
        ...
        private bool TryGenerateMap(int seed, MapLayoutTemplate templateToUse)
        {
            ...
            // Why: NavMeshBaker에서 맵 크기를 알 수 있도록 저장
                                          ...
                                          HashSet<(Vector2Int, Vector2Int)> essentialConnections)
        {
            ...
            for (int cellIndex = 0; cellIndex < cellHasNeighbor.Length; cellIndex++)
            {
                ...
                if (cellHasNeighbor[cellIndex])
                {
                    ...
                    {
                        cellOffset = new Vector2Int(offset.x, offset.y + cellIndex);
                    }

                    Vector2Int neighborGridPos = representativeKey + cellOffset;
                    ...
                    if (generateDoor)
                    {
                        ...
                        {
                            chunk.SetWall(direction, chunk.ChunkData.WallPrefab, _settings.ChunkSize,
                                        chunk.ChunkData.WallSpacing, startWallIndex, midIndex);
                        }

                        // 가운데 문 (1개)
                        chunk.SetDoor(direction, chunk.ChunkData.DoorPrefab, _settings.ChunkSize,
                                    chunk.ChunkData.WallSpacing, midIndex, midIndex + 1);

                        // 문 뒤쪽 벽
                        if (midIndex + 1 < endWallIndex)
                        {
                            chunk.SetWall(direction, chunk.ChunkData.WallPrefab, _settings.ChunkSize,
                                        chunk.ChunkData.WallSpacing, midIndex + 1, endWallIndex);
                        }
                    }
                    else
                    {
