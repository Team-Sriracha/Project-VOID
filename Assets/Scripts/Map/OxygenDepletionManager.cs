...
namespace ProjectVoid.Map
{
    ...
    public class OxygenDepletionManager : NetworkBehaviour
    {
        ...
        private const float MIN_DANGER_ZONE_VOLUME = 0.01f;
        ...
        [Header("사운드")]
        [Tooltip("로컬 플레이어가 산소 고갈 구역에 있을 때 재생할 맵 앰비언스")]
        [SerializeField] private AudioCue _dangerZoneAmbientAudioCue;
        [SerializeField] [Min(0.01f)] private float _dangerZoneFadeDuration = 0.35f;
        ...
        private AudioSource _dangerZoneAmbientSource;
        private float _dangerZoneCurrentVolume;
            ...
            foreach (var kvp in _chunkToIndexMap)
            {
                if (kvp.Value == index)
                {
                    ...
                    if (!_mapGenerator.PlacedChunks.TryGetValue(chunkPos, out var chunkInstance))
                        continue;
                    Vector2Int representativePos = chunkInstance.GridPosition;
                    
                    if (newValue)
                    {
                        _warningChunks.Add(chunkPos);
                        
                        // 대표 위치 기준으로 조명 관리 (SpawnRedLightsForChunk 내부에서 중복 방지)
                        if (!_redLights.ContainsKey(representativePos))
                        {
        ...
        private void PreviewNextPhaseWarning(int nextPhase)
        {
            ...
            {
                Vector2Int representativePos = chunk.GridPosition;
                ...
                // 대표 위치 기준으로 조명 생성 (중복 방지는 SpawnRedLightsForChunk 내부에서)
                if (!_redLights.ContainsKey(representativePos))
                {
                    SpawnRedLightsForChunk(representativePos);
                    Debug.Log($"[OxygenDepletionManager] 경고 Red Light 스폰: Chunk {representativePos}");
                }
            }
        ...
        private void UpdateLocalDangerZoneAmbient()
        {
            if (!IsClientInitialized || Application.isBatchMode || _dangerZoneAmbientAudioCue == null)
            {
                return;
            }

            EnsureDangerZoneAmbientSource();
            if (_dangerZoneAmbientSource == null)
            {
                return;
            }
            ...
            if (_gameStateManager != null &&
                _gameStateManager.CurrentGameState.Value == GameState.Playing &&
                _mapGenerator != null &&
                _chunkToIndexMap != null &&
                _chunkToIndexMap.Count > 0)
            {
                PlayerCombat localCombat = PlayerUtils.GetLocalPlayerCombat();
                if (localCombat != null && localCombat.IsAlive)
                {
                    Vector2Int chunkPos = WorldToChunkPosition(localCombat.transform.position);
                    shouldPlay = IsChunkDepleted(chunkPos);
                }
            }

            float fadeSpeed = 1f / Mathf.Max(0.01f, _dangerZoneFadeDuration);
            float targetVolume = shouldPlay ? ResolveDangerZoneAmbientTargetVolume() : 0f;
            _dangerZoneCurrentVolume = Mathf.MoveTowards(_dangerZoneCurrentVolume, targetVolume, Time.deltaTime * fadeSpeed * Mathf.Max(1f, targetVolume));
            _dangerZoneAmbientSource.volume = _dangerZoneCurrentVolume;

            if (shouldPlay && !_dangerZoneAmbientSource.isPlaying)
            {
                _dangerZoneAmbientSource.Play();
            }
            else if (!shouldPlay && _dangerZoneAmbientSource.isPlaying && _dangerZoneCurrentVolume <= MIN_DANGER_ZONE_VOLUME)
            {
                _dangerZoneAmbientSource.Stop();
            }
        }

        private void EnsureDangerZoneAmbientSource()
        {
            if (_dangerZoneAmbientSource != null)
            {
                return;
            ...
            if (!_dangerZoneAmbientAudioCue.TryGetClip(out AudioClip clip))
            {
                return;
            }

            GameObject ambientObject = new GameObject("DangerZoneAmbient");
            ambientObject.transform.SetParent(transform, false);
            _dangerZoneAmbientSource = ambientObject.AddComponent<AudioSource>();
            _dangerZoneAmbientSource.playOnAwake = false;
            _dangerZoneAmbientSource.loop = true;
            _dangerZoneAmbientSource.spatialBlend = 0f;
            _dangerZoneAmbientSource.dopplerLevel = 0f;
            _dangerZoneAmbientSource.clip = clip;
            _dangerZoneAmbientSource.pitch = _dangerZoneAmbientAudioCue.GetRandomPitch();
            _dangerZoneAmbientSource.volume = 0f;
            _dangerZoneCurrentVolume = 0f;
        }

        private float ResolveDangerZoneAmbientTargetVolume()
        {
            if (_dangerZoneAmbientAudioCue == null)
            {
                return 0f;
            }
            ...
            float sfxVolume = manager != null ? manager.SfxVolume : 1f;
            return masterVolume * sfxVolume * _dangerZoneAmbientAudioCue.Volume;
        }
        ...
        private void UpdateRedLightVisuals()
        {
            ...
            int depletedCount = 0;
            var processedChunks = new HashSet<Vector2Int>(); // 대표 위치 중복 처리 방지
            ...
            foreach (var kvp in _chunkToIndexMap)
            {
                ...
                if (index >= 0 && index < MAX_CHUNK_CAPACITY && _depletedZoneMask[index])
                {
                    ...
                    // 이미 처리한 청크는 스킵
                    if (processedChunks.Contains(representativePos))
                        continue;
                    processedChunks.Add(representativePos);
                    ...
                    EnsureLightsEnabled(representativePos);
                }
            }
            ...
            {
                Vector2Int representativePos = kvp.Key;

                // 경고 중인 청크는 제거하지 않음 (대표 위치로 확인)
                bool isWarning = false;
                if (_mapGenerator.PlacedChunks.TryGetValue(representativePos, out var chunkInstance))
                {
                    // 청크의 모든 그리드 위치 중 하나라도 경고 상태면 유지
                    for (int dy = 0; dy < chunkInstance.ChunkData.ChunkHeight && !isWarning; dy++)
                    {
                        for (int dx = 0; dx < chunkInstance.ChunkData.ChunkWidth && !isWarning; dx++)
                        {
                            Vector2Int cellPos = representativePos + new Vector2Int(dx, dy);
                            if (_warningChunks.Contains(cellPos))
                                isWarning = true;
                        }
                    }
                }
                ...
                // 고갈된 청크는 제거하지 않음
                if (_chunkToIndexMap.TryGetValue(representativePos, out int index))
                {
                    if (index >= 0 && index < MAX_CHUNK_CAPACITY && _depletedZoneMask[index])
                        continue; // 고갈 상태면 조명 유지
                }

                // 경고도 고갈도 아닌 청크는 조명 제거
                chunksToRemove.Add(representativePos);
            }
        ...
        private void SpawnRedLightsForChunk(Vector2Int chunkPos)
        {
            ...
            // 그룹 청크의 경우 대표 위치(GridPosition) 기준으로 처리
            Vector2Int representativePos = chunkInstance.GridPosition;
            
            // 이미 해당 청크의 조명이 생성되어 있으면 스킵 (중복 방지)
            if (_redLights.ContainsKey(representativePos))
                return;
            ...
            // 대표 위치 기준으로 월드 좌표 계산
            Vector3 chunkWorldPos = new Vector3(representativePos.x * chunkSize, 0, representativePos.y * chunkSize);
            ...
            for (int gridY = 0; gridY < chunkHeight; gridY++)
            {
                for (int gridX = 0; gridX < chunkWidth; gridX++)
                {
                    ...
                    lightObj.name = $"RedLight_Chunk{representativePos}_Grid{gridX}x{gridY}";
                    lights.Add(lightObj);
                }
            }

            _redLights[representativePos] = lights;
        }
        ...
        private void OnDestroy()
        {
            if (_dangerZoneAmbientSource != null)
            {
                Destroy(_dangerZoneAmbientSource.gameObject);
                _dangerZoneAmbientSource = null;
            }
