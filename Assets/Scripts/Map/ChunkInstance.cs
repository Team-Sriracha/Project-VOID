...
{
    /// <summary>
    /// Chunk GameObject 컴포넌트. 3D 벽/문 관리
    /// </summary>
    public class ChunkInstance : MonoBehaviour
    {
        ...
        /// 패턴: 지정된 범위의 중앙에 문만 배치 (주변 벽은 별도 SetWall 호출에서 처리)
        /// </summary>
        /// <param name="wallPrefab">사용되지 않음 (하위 호환성을 위해 유지)</param>
        /// <param name="startWallIndex">시작 벽 인덱스 (기본값: 0)</param>
        /// <param name="endWallIndex">종료 벽 인덱스 (기본값: -1, 전체)</param>
        public void SetDoor(Direction direction, GameObject doorPrefab, GameObject wallPrefab, int chunkSize, float wallSpacing, int startWallIndex = 0, int endWallIndex = -1)
        {
            // 청크 크기에 따라 배치할 벽 길이 계산 (멀티 칸 청크 고려)
            int effectiveLength = GetEffectiveLengthForDirection(direction, chunkSize);
            ...
            // 문만 배치 (주변 벽은 별도 SetWall 호출에서 처리됨)
            if (doorPrefab != null)
            {
                Vector3 worldPosition = CalculateWallWorldPosition(direction, centerIndex, effectiveLength, chunkSize, wallSpacing);
                Quaternion rotation = CalculateWallRotation(direction);
        ...
        private Vector3 CalculateWallWorldPosition(Direction direction, int index, int effectiveLength, int chunkSize, float wallSpacing)
        {
            ...
            {
                Direction.North => new Vector3(floorCenterX + positionAlongEdge, 0.5f, height * chunkSize),
                Direction.South => new Vector3(floorCenterX + positionAlongEdge, 0.5f, 0),
                Direction.East => new Vector3(width * chunkSize, 0.5f, floorCenterZ + positionAlongEdge),
                Direction.West => new Vector3(0, 0.5f, floorCenterZ + positionAlongEdge),
                _ => Vector3.zero
            };
        ...
        /// </summary>
        public int GetEffectiveLengthForDirection(Direction direction, int chunkSize)
        {
            if (_chunkData == null)
                return chunkSize;
