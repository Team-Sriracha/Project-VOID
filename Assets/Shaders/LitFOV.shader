Shader "Custom/Lit FOV"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma fragment LitPassFragmentFOV
            ...
            half4 LitPassFragmentFOV(Varyings input) : SV_Target
            {
                ...
                // _FOVEnabled is set by FOVController, defaults to 0 in preview/scene
                if (_FOVEnabled > 0.5 && _FOVRange > 1.0)
                {
                    float3 posWS = input.positionWS;
                    float dist = length(posWS.xz - _FOVCenter.xz);
                    float dimStart = _FOVRange - _FOVEdgeSoftness - _FadeWidth;
                    float dimFactor = saturate((dist - dimStart) / max(_FOVEdgeSoftness, 0.1));
                    color.rgb = lerp(color.rgb, color.rgb * _DimColor.rgb, dimFactor);
                }
