Shader "Custom/Toon FOV"
{
    Properties
    {
        ...
        // [FIX] Must be defined for Stencil block usage. Default to 8 (Always).
        [HideInInspector] _GlobalStencilComp ("Global Stencil Comp", Float) = 8
    }

    SubShader
    {
        Tags { "RenderType"="Opaque" "RenderPipeline"="UniversalPipeline" "Queue"="AlphaTest+100" }
        
        // Use Global Uniform [_GlobalStencilComp].
        Stencil { Ref 1 Comp [_GlobalStencilComp] }
        ...
        Pass
        {
            ...
            float4 _MainTex_ST;
            half4 _Color, _ShadowColor, _DimColor;
            half _ShadowThreshold, _FadeWidth;
            float3 _FOVCenter;
            float _FOVRange, _FOVEdgeSoftness;

            struct Attributes { float4 pos : POSITION; float3 normal : NORMAL; float2 uv : TEXCOORD0; };
            struct Varyings { float4 pos : SV_POSITION; float2 uv : TEXCOORD0; float3 normal : TEXCOORD1; float3 worldPos : TEXCOORD2; };
            ...
            half4 frag(Varyings i) : SV_Target
            {
                ...
                half3 result = col.rgb * toon;
                
                if (_FOVRange > 0.1)
                {
                    float dist = length(i.worldPos.xz - _FOVCenter.xz);
                    float dimStart = _FOVRange - _FOVEdgeSoftness - _FadeWidth;
                    result = lerp(result, result * _DimColor.rgb, saturate((dist - dimStart) / max(_FOVEdgeSoftness, 0.1)));
                }
